...
// Use the integrated Python service for proper .mind file generation
async function generateMindFileWithPythonService(imageUrl: string): Promise<ArrayBuffer | null> {
  try {
    // Use the integrated Python service URL
    ...
    // Send to integrated Python service for .mind file generation
    const mindResponse = await fetch(`${pythonServiceUrl}/generate-mind`, {
      method: 'POST',
      headers: {
    ...
    if (!mindResponse.ok) {
      const errorText = await mindResponse.text()
      throw new Error(`Python service error: ${mindResponse.status} - ${errorText}`)
    }
    
    const mindFileBuffer = await mindResponse.arrayBuffer()
    console.log('MindAR file generated by integrated Python service, size:', mindFileBuffer.byteLength)
    
    return mindFileBuffer
    
  } catch (error) {
    console.error('Integrated Python service error:', error)
    return null
  }
}

// Fallback: Create a basic MindAR file if Python service is not available
function createBasicMindARFile(imageBuffer: ArrayBuffer): Uint8Array {
  const imageData = new Uint8Array(imageBuffer)
  
  // Create a minimal but valid MindAR file structure
...
export async function POST(request: NextRequest) {
  try {
    ...
    let mindFile: Uint8Array | null = null

    // Try integrated Python service first
    const pythonMindFile = await generateMindFileWithPythonService(imageUrl)
    if (pythonMindFile) {
      mindFile = new Uint8Array(pythonMindFile)
      console.log('✅ Using integrated Python service generated MindAR file')
    } else {
      // Fallback to basic generation
      console.log('⚠️ Integrated Python service not available, using fallback generation')
      ...
      // Generate basic MindAR file
      mindFile = createBasicMindARFile(imageBuffer)
      console.log('Basic MindAR file created, size:', mindFile.length)
    }

    if (!mindFile) {
      throw new Error('Failed to generate MindAR file')
    }
    ...
    return NextResponse.json({
      ...
      method: pythonMindFile ? 'integrated-python-service' : 'fallback'
    })
