...
CREATE TABLE ar_analytics_events (
...
CREATE TABLE ar_analytics_sessions (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE ar_analytics_daily_aggregates (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  date DATE NOT NULL,
  experience_id VARCHAR(255) NOT NULL,
...
CREATE TABLE ar_analytics_conversions (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  experience_id VARCHAR(255) NOT NULL,
  user_id UUID REFERENCES auth.users(id),
  session_id VARCHAR(255) NOT NULL,
...
CREATE TABLE ar_analytics_performance (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  experience_id VARCHAR(255) NOT NULL,
  session_id VARCHAR(255) NOT NULL,
...
CREATE TABLE ar_analytics_geographic (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  experience_id VARCHAR(255) NOT NULL,
  session_id VARCHAR(255) NOT NULL,
  country VARCHAR(100),
...
CREATE INDEX CONCURRENTLY idx_analytics_sessions_composite 
...
CREATE UNIQUE INDEX ON analytics_dashboard_metrics (user_id);
...
-- Schedule refresh (you can set up a cron job or use pg_cron extension)
-- SELECT cron.schedule('refresh-analytics', '*/15 * * * *', 'SELECT refresh_analytics_dashboard();');
